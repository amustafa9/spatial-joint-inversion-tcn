...
from core.datasets import SeismicDataset1D, UnsupervisedSeismicDataset
from torch.utils.data import DataLoader
from core.model1D import inverse_model, forward_model
...
def train(**kwargs):
    ...
    seam_unsupervised_dataset = UnsupervisedSeismicDataset(seismic, np.arange(len(seismic)))
    seam_unsupervised_loader = DataLoader(seam_unsupervised_dataset, batch_size = 400)
    ...
    # set up models
    inverse_net = inverse_model(resolution_ratio=1, nonlinearity='relu').to(device)
    forward_net = forward_model(resolution_ratio=1).to(device)
    
    # Set up loss
    criterion = torch.nn.MSELoss()
    
    
    optimizer_seam = torch.optim.Adam(list(inverse_net.parameters())+list(forward_net.parameters()), 
                                      amsgrad=True,
                                      lr=0.005)
    ...
    for epoch in range(kwargs['epochs']):
      ...
      for x,y in seam_train_loader:
        ...
        loss_train = criterion(y_pred, y) + criterion(x, x_rec) 
        
    
      loss_train.backward()
      
      for x in seam_unsupervised_loader:
          x_rec = forward_net(inverse_net(x))
          unsupervised_loss = criterion(x, x_rec)
          
      unsupervised_loss.backward()
      
      optimizer_seam.step()
      
      print('Epoch: {} | Supervised Loss: {:0.4f} | Unsupervised Loss : {:0.4f}'\
            .format(epoch, loss_train.item(), unsupervised_loss.item()))
    ...
    torch.save(inverse_net.state_dict(), 'saved_models/model_seam_1D.pth')
...
def test(**kwargs):
    ...
    # set up models
    model_seam = inverse_model().to(device)
